...
Este documento define o que foi construído na Parte 1 do teste, por que essa funcionalidade
foi escolhida e onde estão os limites do escopo.

## 1. Funcionalidade escolhida

Cadastro e gestão de processos jurídicos (entidade `LegalCase` no código).

O enunciado deixa a funcionalidade em aberto ("implemente uma funcionalidade ponta a ponta
em um módulo do produto"), então definir o escopo já faz parte do desafio. Escolhi um módulo
de processos jurídicos por três motivos:

1. Aderência ao domínio. Gestão de processos é o núcleo de uma procuradoria digital.
2. Riqueza de regras de negócio. Dá para demonstrar validação de domínio real (número CNJ) e
   regra de negócio além da validação de campo (transições de status).
3. Escopo controlável. Uma única entidade bem modelada cobre todos os itens exigidos (front com
   fluxo completo, API documentada, persistência, logs e testes) sem inflar o projeto.
...
- **RF06** Excluir um processo por exclusão lógica (soft delete). Processos jurídicos carregam
  expectativa de retenção e auditoria, então o registro é marcado como removido em vez de apagado
  fisicamente.

## 3. Modelo de domínio

Entidade principal: `LegalCase`. Os nomes seguem o código (em inglês).
...
**RN01 — Validação do número CNJ.** O número deve seguir o formato `NNNNNNN-DD.AAAA.J.TR.OOOO`
e ter dígito verificador válido (ISO 7064, MOD 97-10).

**RN02 — Unicidade do número CNJ.** Não pode haver dois processos com o mesmo número. A regra é
validada na aplicação e também por constraint única no banco. A unicidade é global, inclusive
sobre registros removidos logicamente: um CNJ é único para sempre.
...
`ARCHIVED` e `CLOSED` são estados terminais. Qualquer transição fora dessas (incluindo
auto-transição) é rejeitada com `422`.
...
- **RNF04** Testes (unitários) cobrindo os cenários principais.
- **RNF05** Projeto executável com poucos comandos (Docker Compose) e README claro.

## 6. Fora de escopo

Decisões conscientes para manter o foco dentro do prazo (detalhadas na nota técnica):

- Autenticação e autorização.
- Auditoria de autoria (`created_by`/`updated_by`). Depende de autenticação; o "quando" já
  existe (`createdAt`/`updatedAt`), só o "quem" ficou de fora.
- Multi-tenant e multiusuário.
- Anexos e upload de documentos do processo.
...
A Parte 2 reaproveita a instrumentação de logs da Parte 1: o incidente analisado é um cenário do próprio módulo (ver [`ANALISE-INCIDENTE.md`](ANALISE-INCIDENTE.md)).
